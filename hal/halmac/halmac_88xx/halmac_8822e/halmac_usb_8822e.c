...
#if (HALMAC_8822E_SUPPORT && HALMAC_USB_SUPPORT)

/**
 * mac_pwr_switch_usb_8822e() - switch mac power
...
mac_pwr_switch_usb_8822e(struct halmac_adapter *adapter,
 ...
 */
enum halmac_ret_status
phy_cfg_usb_8822e(struct halmac_adapter *adapter,
		  enum halmac_intf_phy_platform pltfm)
{
	...
	status = parse_intf_phy_88xx(adapter, usb2_phy_param_8822e, pltfm,
				     HAL_INTF_PHY_USB2);

	if (status != HALMAC_RET_SUCCESS)
		return status;

	status = parse_intf_phy_88xx(adapter, usb3_phy_param_8822e, pltfm,
				     HAL_INTF_PHY_USB3);
...
/**
 * halmac_pcie_switch_8822e() - pcie gen1/gen2 switch
 * @adapter : the adapter of halmac
 ...
 */
enum halmac_ret_status
pcie_switch_usb_8822e(struct halmac_adapter *adapter, enum halmac_pcie_cfg cfg)
{
	return HALMAC_RET_NOT_SUPPORT;
}

/**
 * intf_tun_usb_8822e() - usb interface fine tuning
 * @adapter : the adapter of halmac
 * Author : Ivan
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
intf_tun_usb_8822e(struct halmac_adapter *adapter)
{
	return HALMAC_RET_SUCCESS;
}
