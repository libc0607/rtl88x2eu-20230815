...
#define GPIO15_SW_IO_8822E	\
	{HALMAC_SW_IO, HALMAC_GPIO15, HALMAC_GPIO_IN_OUT, \
	 0x40, BIT(1) | BIT(0), 0}
/* EECS definition */
#define EECS_BT_GPIO17_8822E	\
	{HALMAC_BT_GPIO, HALMAC_EECS, HALMAC_GPIO_IN_OUT, \
	 0x67, BIT(1), BIT(1)}
#define EECS_RFE_CTRL_7_8822E	\
	{HALMAC_WLPHY_RFE_CTRL2GPIO_2, HALMAC_EECS, HALMAC_GPIO_IN_OUT, \
	 0x40, BIT(3), BIT(3)}
...
static const struct halmac_gpio_pimux_list PINMUX_LIST_EECS_8822E[] = {
	EECS_BT_GPIO17_8822E,
	EECS_RFE_CTRL_7_8822E
};
		      ...
		      u32 *list_size, u32 *gpio_id)
{
	...
	case HALMAC_GPIO_FUNC_SW_IO_15:
		...
		*gpio_id = HALMAC_GPIO15;
		break;
	case HALMAC_GPIO_FUNC_RFE_CTRL_7:
		*list = PINMUX_LIST_EECS_8822E;
		*list_size = ARRAY_SIZE(PINMUX_LIST_EECS_8822E);
		*gpio_id = HALMAC_EECS;
		break;
	case HALMAC_GPIO_FUNC_S1_PAPE:
	case HALMAC_GPIO_FUNC_S1_TRSWB:
		       ...
		       enum halmac_gpio_func gpio_func)
{
	...
	case HALMAC_GPIO_FUNC_S1_TRSW:
	case HALMAC_GPIO_FUNC_RFE_CTRL_11:
		if (info->sw_io_2 == 1 || info->s1_trsw == 1 || info->rfe_ctrl_11 == 1)
			status = HALMAC_RET_PINMUX_USED;
		break;
	case HALMAC_GPIO_FUNC_SW_IO_3:
	case HALMAC_GPIO_FUNC_S0_TRSW:
	case HALMAC_GPIO_FUNC_RFE_CTRL_3:
		if (info->sw_io_3 == 1 || info->s0_trsw == 1 || info->rfe_ctrl_3 == 1)
			status = HALMAC_RET_PINMUX_USED;
		break;
	case HALMAC_GPIO_FUNC_SW_IO_4:
	case HALMAC_GPIO_FUNC_RFE_CTRL_8:
		if (info->sw_io_4 == 1 || info->rfe_ctrl_8 == 1)
			status = HALMAC_RET_PINMUX_USED;
		break;
	case HALMAC_GPIO_FUNC_SW_IO_5:
		if (info->sw_io_5 == 1)
	...
	case HALMAC_GPIO_FUNC_RFE_CTRL_9:
		if (info->sw_io_13 == 1 || info->bt_dev_wake == 1 ||
		    info->antsw == 1 || info->rfe_ctrl_9 == 1)
			status = HALMAC_RET_PINMUX_USED;
		break;
	case HALMAC_GPIO_FUNC_BT_DEV_WAKE1:
		if (info->sw_io_13 == 1 || info->bt_dev_wake == 1 ||
		    info->wl_led == 1 || info->antsw == 1 || info->rfe_ctrl_9 == 1)
			status = HALMAC_RET_PINMUX_USED;
		break;
	case HALMAC_GPIO_FUNC_SW_IO_14:
	case HALMAC_GPIO_FUNC_RFE_CTRL_5:
		if (info->sw_io_14 == 1 || info->bt_host_wake == 1 || info->rfe_ctrl_5 == 1)
			status = HALMAC_RET_PINMUX_USED;
		break;
	case HALMAC_GPIO_FUNC_BT_HOST_WAKE1:
		    ...
		    u32 gpio_id, enum halmac_gpio_func gpio_func)
{
	...
	case HALMAC_GPIO_FUNC_RFE_CTRL_9:
		...
		break;
	case HALMAC_GPIO_FUNC_RFE_CTRL_7:
		switch_func = HALMAC_WLPHY_RFE_CTRL2GPIO_2;
		break;
	default:
		return HALMAC_RET_SWITCH_CASE_ERROR;
	}
	...
	*state = HALMAC_GPIO_CFG_STATE_BUSY;
	...
	case HALMAC_WL_DPDT_SEL:
		...
		} else if (gpio_id == HALMAC_GPIO4) {
			HALMAC_REG_W8_SET(REG_GPIO_MUXCFG + 3, BIT(2));
	...
	case HALMAC_WLPHY_RFE_CTRL2GPIO:
		...
		else if (gpio_id == HALMAC_GPIO14)
			HALMAC_REG_W8_SET(REG_GPIO_MUXCFG_2, BIT(2));
		break;
	default:
		break;
	}
		    ...
		    enum halmac_gpio_func gpio_func, u8 val)
{
	...
	case HALMAC_GPIO_FUNC_ANTSWB:
		...
		break;
	case HALMAC_GPIO_FUNC_RFE_CTRL_3:
		adapter->pinmux_info.rfe_ctrl_3 = val;
		break;
	case HALMAC_GPIO_FUNC_RFE_CTRL_5:
		adapter->pinmux_info.rfe_ctrl_5 = val;
		break;
	case HALMAC_GPIO_FUNC_RFE_CTRL_7:
		adapter->pinmux_info.rfe_ctrl_7 = val;
		break;
	case HALMAC_GPIO_FUNC_RFE_CTRL_8:
		adapter->pinmux_info.rfe_ctrl_8 = val;
		break;
	case HALMAC_GPIO_FUNC_RFE_CTRL_9:
		adapter->pinmux_info.rfe_ctrl_9 = val;
		break;
	case HALMAC_GPIO_FUNC_RFE_CTRL_11:
		adapter->pinmux_info.rfe_ctrl_11 = val;
		break;
	default:
		return HALMAC_RET_GET_PINMUX_ERR;
	}
