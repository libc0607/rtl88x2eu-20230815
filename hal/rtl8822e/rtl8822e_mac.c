 ...
 *
 *****************************************************************************/
#define _RTL8822E_MAC_C_

#include <drv_types.h>		/* PADAPTER, basic_types.h and etc. */
#include <hal_data.h>		/* HAL_DATA_TYPE */
#include "../hal_halmac.h"	/* Register Definition and etc. */
#include "rtl8822e.h"		/* FW array */


inline u8 rtl8822e_rcr_config(PADAPTER p, u32 rcr)
{
	u32 v32;
	int err;
	...
	v32 ^= rcr;
	v32 &= BIT_APP_PHYSTS_8822E;
	if (v32) {
		v32 = rcr & BIT_APP_PHYSTS_8822E;
	...
	err = rtw_write32(p, REG_RCR_8822E, rcr);
	if (_FAIL == err)
		return _FALSE;
...
inline u8 rtl8822e_rx_ba_ssn_appended(PADAPTER p)
{
	return rtw_hal_rcr_check(p, BIT_APP_BASSN_8822E);
}

inline u8 rtl8822e_rx_fcs_append_switch(PADAPTER p, u8 enable)
{
	u32 rcr_bit;
	u8 ret = _TRUE;

	rcr_bit = BIT_APP_FCS_8822E;
	if (_TRUE == enable)
...
inline u8 rtl8822e_rx_fcs_appended(PADAPTER p)
{
	return rtw_hal_rcr_check(p, BIT_APP_FCS_8822E);
}

inline u8 rtl8822e_rx_tsf_addr_filter_config(PADAPTER p, u8 config)
{
	u8 v8;
	int err;
	...
	if (v8 != config) {
		...
		err = rtw_write8(p, REG_NAN_RX_TSF_FILTER_8822E, config);
		if (_FAIL == err)
			return _FALSE;
	}
 ...
 */
s32 rtl8822e_fw_dl(PADAPTER adapter, u8 wowlan)
{
...
#endif /* CONFIG_FILE_FWIMG */
	{
		#ifdef CONFIG_WOWLAN
		if (_TRUE == wowlan)
			err = rtw_halmac_dlfw(d, array_mp_8822e_fw_wowlan, array_length_mp_8822e_fw_wowlan);
		else
		#endif /* CONFIG_WOWLAN */
			err = rtw_halmac_dlfw(d, array_mp_8822e_fw_nic, array_length_mp_8822e_fw_nic);
	}
...
{
	struct dvobj_priv *d;
	u32 size = 48;	/* HALMAC_TX_DESC_SIZE_8822E */
	int err = 0;
...
{
	struct dvobj_priv *d;
	u32 size = 24;	/* HALMAC_RX_DESC_SIZE_8822E */
	int err = 0;
