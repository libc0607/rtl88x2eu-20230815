 ...
 *
 *****************************************************************************/
#define _RTL8822EU_HALINIT_C_

#include <hal_data.h>			/* HAL DATA */
#include "../../hal_halmac.h"		/* HALMAC API */
#include "../rtl8822e.h"		/* rtl8822e hal common define */
#include "rtl8822eu.h"
...
u8 rtl8822eu_fw_ips_init(_adapter *padapter)
{
...
send_fwips_h2c:
		...
		rtl8822e_set_FwPwrModeInIPS_cmd(padapter, 0);
...
u8 rtl8822eu_fw_ips_deinit(_adapter *padapter)
{
	struct sreset_priv *psrtpriv =  &GET_HAL_DATA(padapter)->srestpriv;
	struct debug_priv *pdbgpriv = &adapter_to_dvobj(padapter)->drv_dbg;
	struct pwrctrl_priv *pwrctl = adapter_to_pwrctl(padapter);
	...
	if (pwrctl->bips_processing == _TRUE && psrtpriv->silent_reset_inprogress == _FALSE
		...
		rtl8822e_set_FwPwrModeInIPS_cmd(padapter, 0x1);
#ifdef CONFIG_LPS_LCLK
		if (rtw_is_fw_ips_lclk_mode(padapter) == _FALSE)
			return _SUCCESS;
...
u32 rtl8822eu_init(PADAPTER padapter)
{
	u8 status = _SUCCESS;
	...
	if (rtw_is_fw_ips_mode(padapter) == _TRUE) {
		if (_SUCCESS == rtl8822eu_fw_ips_init(padapter))
			goto exit;
	}
#endif

	if (rtl8822e_init(padapter) == _FAIL)
		status = _FAIL;
	else
...
u32 rtl8822eu_deinit(PADAPTER padapter)
{
	struct pwrctrl_priv *pwrctl = adapter_to_pwrctl(padapter);
...
#ifdef CONFIG_FWLPS_IN_IPS
	if (rtw_is_fw_ips_mode(padapter) == _TRUE) {
		if (_SUCCESS == rtl8822eu_fw_ips_deinit(padapter))
			goto exit;
	}
#endif

	hal_deinit_misc(padapter);
	status = rtl8822e_deinit(padapter);
	if (status == _FALSE) {
		RTW_INFO("%s: rtl8822e_hal_deinit fail\n", __func__);
		return _FAIL;
	}
...
u32 rtl8822eu_inirp_init(PADAPTER padapter)
{
...
u32 rtl8822eu_inirp_deinit(PADAPTER padapter)
{
	struct recv_priv	*precvpriv = &padapter->recvpriv;
	_pkt			*pskb;
...
void rtl8822eu_update_interrupt_mask(PADAPTER padapter, u8 bHIMR0 , u32 AddMSR, u32 RemoveMSR)
{
	...
	if (bHIMR0)
		rtw_write32(padapter, REG_HIMR0_8822E, *himr);
	else
		rtw_write32(padapter, REG_HIMR1_8822E, *himr);
...
void rtl8822eu_interface_configure(PADAPTER padapter)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
